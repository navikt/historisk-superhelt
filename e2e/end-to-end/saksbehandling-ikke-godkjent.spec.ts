...
test.describe("Saksbehandling og attestering ikke godkjent", () => {
    ...
    test.beforeAll(async () => {
        console.debug(`Generert fødselsnummer for "Saksbehandling og attestering ikke godkjent" testene: ${brukerFnr}`);
    });
    ...
    test("Avvis attestering som Atle Attestant", async ({ page, auth, sok, sak }) => {
        ...
        await test.step("Attester og avvis", async () => {
            await test.step("Attester og tilbakefør", async () => {
                ...
                await page.getByRole("radio", { name: "Underkjenn og send tilbake" }).check();

                await page.getByRole("textbox", { name: "Årsak til avslag" }).fill("Dette må bli bedre");
