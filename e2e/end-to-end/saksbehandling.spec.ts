...
test.describe("Superhelt", () => {
  ...
  test("Behandle sak som Sara Saksbehandler", async ({ page, auth }) => {
    ...
    await test.step("Fyll inn opplysninger", async () => {
      await expect(
        page.getByRole("link", { name: "Opplysninger" }),
      ).toHaveClass(/active/);
      await page
    ...
    await test.step("Skriv brev", async () => {
      await expect(
        page.getByRole("link", { name: "Brev til bruker" }),
      ).toHaveClass(/active/);
      await expect(
        page.getByText("Dokumentbeskrivelse i arkivet"),
      ).toBeVisible();
      await page.getByRole("button", { name: "Lagre og gå videre" }).click();
    });
    await test.step("Send til attestering", async () => {
      await expect(
        page.getByRole("link", { name: "Oppsummering" }),
      ).toHaveClass(/active/);
      await expect(
        page.getByRole("heading", { name: "Til attestering" }),
      ).toBeVisible();
