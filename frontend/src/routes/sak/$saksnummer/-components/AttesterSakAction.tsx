...
export default function AttesterSakAction({ sak }: Props) {
    ...
    function validate() {
        ...
        const kommentarValid =
            beslutning === "godkjent" || (beslutning === "ikke_godkjent" && kommentar.trim().length > 5);
        setValidering({
        ...
        <Card>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <Dialog.Popup closeOnOutsideClick={false}>
                    ...
                    <Dialog.Body>
                        <BodyLong>
                            Denne saken er underkjent og sendt tilbake til saksbehandler for videre arbeid. Det
                            opprettes en ny oppgave som legges til oppgavebenken til saksbehandleren. Når
                            saksbehandleren har gjort nødvendige endringer og sendt saken på nytt, vil den komme tilbake
                            til attestering.
                        </BodyLong>
                    </Dialog.Body>
                    <Dialog.Footer>
                        <Button as={Link} to={"/"} onClick={() => setOpen(false)}>
                            Gå til din oppgavebenk
                        </Button>
                    </Dialog.Footer>
                </Dialog.Popup>
            </Dialog>
