// This file is auto-generated by @hey-api/openapi-ts

import type { Client, Options as Options2, TDataShape } from './client';
import { client } from './client.gen';
import type { AttersterSakData, AttersterSakErrors, AttersterSakResponses, FeilregisterSakData, FeilregisterSakErrors, FeilregisterSakResponses, FerdigstillSakData, FerdigstillSakErrors, FerdigstillSakResponses, FindPersonByFnrData, FindPersonByFnrErrors, FindPersonByFnrResponses, FindSakerForPersonData, FindSakerForPersonErrors, FindSakerForPersonResponses, FinnJournalposterForBrukerData, FinnJournalposterForBrukerErrors, FinnJournalposterForBrukerResponses, FinnJournalposterForSakData, FinnJournalposterForSakErrors, FinnJournalposterForSakResponses, GetKodeverkHjemlerData, GetKodeverkHjemlerErrors, GetKodeverkHjemlerResponses, GetKodeverkHjemlerForYtelseData, GetKodeverkHjemlerForYtelseErrors, GetKodeverkHjemlerForYtelseResponses, GetKodeverkSakStatusData, GetKodeverkSakStatusErrors, GetKodeverkSakStatusResponses, GetKodeverkStonadTypeData, GetKodeverkStonadTypeErrors, GetKodeverkStonadTypeResponses, GetKodeverkVedtaksResultatData, GetKodeverkVedtaksResultatErrors, GetKodeverkVedtaksResultatResponses, GetOppgaveData, GetOppgaveErrors, GetOppgaveResponses, GetPersonByMaskertIdentData, GetPersonByMaskertIdentErrors, GetPersonByMaskertIdentResponses, GetSakBySaksnummerData, GetSakBySaksnummerErrors, GetSakBySaksnummerResponses, GetSakStatusData, GetSakStatusErrors, GetSakStatusResponses, GetUserInfoData, GetUserInfoErrors, GetUserInfoResponses, GjenapneSakData, GjenapneSakErrors, GjenapneSakResponses, GjenopprettOppgaverData, GjenopprettOppgaverErrors, GjenopprettOppgaverResponses, HenleggSakData, HenleggSakErrors, HenleggSakResponses, HentBrevData, HentBrevErrors, HentBrevResponses, HentEllerOpprettBrevData, HentEllerOpprettBrevErrors, HentEllerOpprettBrevResponses, HentEndringsloggForSakData, HentEndringsloggForSakErrors, HentEndringsloggForSakResponses, HentFeileteUtbetalingerData, HentFeileteUtbetalingerErrors, HentFeileteUtbetalingerResponses, HentJournalpostMetaDataData, HentJournalpostMetaDataErrors, HentJournalpostMetaDataResponses, HentOppgaverForPersonData, HentOppgaverForPersonErrors, HentOppgaverForPersonResponses, HentOppgaverForSaksbehandlerData, HentOppgaverForSaksbehandlerErrors, HentOppgaverForSaksbehandlerResponses, HentSakshistorikkForPersonData, HentSakshistorikkForPersonErrors, HentSakshistorikkForPersonResponses, HtmlBrevData, HtmlBrevErrors, HtmlBrevResponses, JournalforKnyttTilEksisterendeSakData, JournalforKnyttTilEksisterendeSakErrors, JournalforKnyttTilEksisterendeSakResponses, JournalforNySakData, JournalforNySakErrors, JournalforNySakResponses, LastnedDokumentFraJournalpostData, LastnedDokumentFraJournalpostErrors, LastnedDokumentFraJournalpostResponses, OppdaterBrevData, OppdaterBrevErrors, OppdaterBrevResponses, OppdaterSakData, OppdaterSakErrors, OppdaterSakResponses, RekjorFeileteUtbetalingerData, RekjorFeileteUtbetalingerErrors, RekjorFeileteUtbetalingerResponses, RetryFeiletUtbetalingData, RetryFeiletUtbetalingErrors, RetryFeiletUtbetalingResponses, SendBrevData, SendBrevErrors, SendBrevResponses, SendKlageTilKabalData, SendKlageTilKabalErrors, SendKlageTilKabalResponses, SendTilAttesteringData, SendTilAttesteringErrors, SendTilAttesteringResponses, TilbakestillGjenapningData, TilbakestillGjenapningErrors, TilbakestillGjenapningResponses } from './types.gen';

export type Options<TData extends TDataShape = TDataShape, ThrowOnError extends boolean = boolean, TResponse = unknown> = Options2<TData, ThrowOnError, TResponse> & {
    /**
     * You can provide a client instance returned by `createClient()` instead of
     * individual options. This might be also useful if you want to implement a
     * custom client.
     */
    client?: Client;
    /**
     * You can pass arbitrary values through the `meta` object. This can be
     * used to access values that aren't defined as part of the SDK function.
     */
    meta?: Record<string, unknown>;
};

/**
 * Hent opp en sak
 */
export const getSakBySaksnummer = <ThrowOnError extends boolean = false>(options: Options<GetSakBySaksnummerData, ThrowOnError>) => (options.client ?? client).get<GetSakBySaksnummerResponses, GetSakBySaksnummerErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}',
    ...options
});

export const oppdaterSak = <ThrowOnError extends boolean = false>(options: Options<OppdaterSakData, ThrowOnError>) => (options.client ?? client).put<OppdaterSakResponses, OppdaterSakErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

export const tilbakestillGjenapning = <ThrowOnError extends boolean = false>(options: Options<TilbakestillGjenapningData, ThrowOnError>) => (options.client ?? client).put<TilbakestillGjenapningResponses, TilbakestillGjenapningErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/status/tilbakestill',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

export const sendTilAttestering = <ThrowOnError extends boolean = false>(options: Options<SendTilAttesteringData, ThrowOnError>) => (options.client ?? client).put<SendTilAttesteringResponses, SendTilAttesteringErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/status/tilattestering',
    ...options
});

export const henleggSak = <ThrowOnError extends boolean = false>(options: Options<HenleggSakData, ThrowOnError>) => (options.client ?? client).put<HenleggSakResponses, HenleggSakErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/status/henlegg',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

export const gjenapneSak = <ThrowOnError extends boolean = false>(options: Options<GjenapneSakData, ThrowOnError>) => (options.client ?? client).put<GjenapneSakResponses, GjenapneSakErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/status/gjenapne',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

/**
 * Rekjøring av utbetaling og brevsending for en ferdig attestert sak
 */
export const ferdigstillSak = <ThrowOnError extends boolean = false>(options: Options<FerdigstillSakData, ThrowOnError>) => (options.client ?? client).put<FerdigstillSakResponses, FerdigstillSakErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/status/ferdigstill',
    ...options
});

export const feilregisterSak = <ThrowOnError extends boolean = false>(options: Options<FeilregisterSakData, ThrowOnError>) => (options.client ?? client).put<FeilregisterSakResponses, FeilregisterSakErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/status/feilregister',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

export const attersterSak = <ThrowOnError extends boolean = false>(options: Options<AttersterSakData, ThrowOnError>) => (options.client ?? client).put<AttersterSakResponses, AttersterSakErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/status/attester',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

export const hentBrev = <ThrowOnError extends boolean = false>(options: Options<HentBrevData, ThrowOnError>) => (options.client ?? client).get<HentBrevResponses, HentBrevErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/brev/{brevId}',
    ...options
});

export const oppdaterBrev = <ThrowOnError extends boolean = false>(options: Options<OppdaterBrevData, ThrowOnError>) => (options.client ?? client).put<OppdaterBrevResponses, OppdaterBrevErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/brev/{brevId}',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

/**
 * Journalfør en journalpost og knytter til ny sak
 */
export const journalforNySak = <ThrowOnError extends boolean = false>(options: Options<JournalforNySakData, ThrowOnError>) => (options.client ?? client).put<JournalforNySakResponses, JournalforNySakErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/dokarkiv/{journalpostId}/journalfor/ny',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

/**
 * Journalfør og knytt til en eksisterende sak
 */
export const journalforKnyttTilEksisterendeSak = <ThrowOnError extends boolean = false>(options: Options<JournalforKnyttTilEksisterendeSakData, ThrowOnError>) => (options.client ?? client).put<JournalforKnyttTilEksisterendeSakResponses, JournalforKnyttTilEksisterendeSakErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/dokarkiv/{journalpostId}/journalfor/eksisterende',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

/**
 * Kjører på nytt  utbetaling som har feilet
 */
export const retryFeiletUtbetaling = <ThrowOnError extends boolean = false>(options: Options<RetryFeiletUtbetalingData, ThrowOnError>) => (options.client ?? client).post<RetryFeiletUtbetalingResponses, RetryFeiletUtbetalingErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/utbetaling/retry/{saksnummer}',
    ...options
});

export const sendKlageTilKabal = <ThrowOnError extends boolean = false>(options: Options<SendKlageTilKabalData, ThrowOnError>) => (options.client ?? client).post<SendKlageTilKabalResponses, SendKlageTilKabalErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/klage',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

export const hentEllerOpprettBrev = <ThrowOnError extends boolean = false>(options: Options<HentEllerOpprettBrevData, ThrowOnError>) => (options.client ?? client).post<HentEllerOpprettBrevResponses, HentEllerOpprettBrevErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/brev',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

export const sendBrev = <ThrowOnError extends boolean = false>(options: Options<SendBrevData, ThrowOnError>) => (options.client ?? client).post<SendBrevResponses, SendBrevErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/brev/{brevId}/send',
    ...options
});

/**
 * Finn person basert på fødselsnummer
 */
export const findPersonByFnr = <ThrowOnError extends boolean = false>(options: Options<FindPersonByFnrData, ThrowOnError>) => (options.client ?? client).post<FindPersonByFnrResponses, FindPersonByFnrErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/person',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
    }
});

/**
 * Hent alle utbetalinger som har feilet
 */
export const hentFeileteUtbetalinger = <ThrowOnError extends boolean = false>(options?: Options<HentFeileteUtbetalingerData, ThrowOnError>) => (options?.client ?? client).get<HentFeileteUtbetalingerResponses, HentFeileteUtbetalingerErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/admin/utbetaling/feilet',
    ...options
});

/**
 * Kjører på nytt  utbetalinger som har feilet
 */
export const rekjorFeileteUtbetalinger = <ThrowOnError extends boolean = false>(options?: Options<RekjorFeileteUtbetalingerData, ThrowOnError>) => (options?.client ?? client).post<RekjorFeileteUtbetalingerResponses, RekjorFeileteUtbetalingerErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/admin/utbetaling/feilet',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options?.headers
    }
});

/**
 * Gjenopprett manglende oppgaver for saker
 */
export const gjenopprettOppgaver = <ThrowOnError extends boolean = false>(options?: Options<GjenopprettOppgaverData, ThrowOnError>) => (options?.client ?? client).post<GjenopprettOppgaverResponses, GjenopprettOppgaverErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/admin/oppgave/gjenopprettOppgaver',
    ...options
});

export const getUserInfo = <ThrowOnError extends boolean = false>(options?: Options<GetUserInfoData, ThrowOnError>) => (options?.client ?? client).get<GetUserInfoResponses, GetUserInfoErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/user',
    ...options
});

export const hentSakshistorikkForPerson = <ThrowOnError extends boolean = false>(options: Options<HentSakshistorikkForPersonData, ThrowOnError>) => (options.client ?? client).get<HentSakshistorikkForPersonResponses, HentSakshistorikkForPersonErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sakshistorikk/person/{maskertPersonIdent}',
    ...options
});

/**
 * Finn saker for en person
 */
export const findSakerForPerson = <ThrowOnError extends boolean = false>(options: Options<FindSakerForPersonData, ThrowOnError>) => (options.client ?? client).get<FindSakerForPersonResponses, FindSakerForPersonErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak',
    ...options
});

/**
 * Hent aggregert status for sak, utbetaling og brev
 */
export const getSakStatus = <ThrowOnError extends boolean = false>(options: Options<GetSakStatusData, ThrowOnError>) => (options.client ?? client).get<GetSakStatusResponses, GetSakStatusErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/status',
    ...options
});

export const hentEndringsloggForSak = <ThrowOnError extends boolean = false>(options: Options<HentEndringsloggForSakData, ThrowOnError>) => (options.client ?? client).get<HentEndringsloggForSakResponses, HentEndringsloggForSakErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/endringslogg',
    ...options
});

export const htmlBrev = <ThrowOnError extends boolean = false>(options: Options<HtmlBrevData, ThrowOnError>) => (options.client ?? client).get<HtmlBrevResponses, HtmlBrevErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/{saksnummer}/brev/{brevId}/html',
    ...options
});

export const getKodeverkVedtaksResultat = <ThrowOnError extends boolean = false>(options?: Options<GetKodeverkVedtaksResultatData, ThrowOnError>) => (options?.client ?? client).get<GetKodeverkVedtaksResultatResponses, GetKodeverkVedtaksResultatErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/kodeverk/vedtaksresultater',
    ...options
});

export const getKodeverkStonadType = <ThrowOnError extends boolean = false>(options?: Options<GetKodeverkStonadTypeData, ThrowOnError>) => (options?.client ?? client).get<GetKodeverkStonadTypeResponses, GetKodeverkStonadTypeErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/kodeverk/stonadtyper',
    ...options
});

export const getKodeverkSakStatus = <ThrowOnError extends boolean = false>(options?: Options<GetKodeverkSakStatusData, ThrowOnError>) => (options?.client ?? client).get<GetKodeverkSakStatusResponses, GetKodeverkSakStatusErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/sak/kodeverk/sakstatuser',
    ...options
});

export const getPersonByMaskertIdent = <ThrowOnError extends boolean = false>(options: Options<GetPersonByMaskertIdentData, ThrowOnError>) => (options.client ?? client).get<GetPersonByMaskertIdentResponses, GetPersonByMaskertIdentErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/person/{maskertPersonident}',
    ...options
});

export const getOppgave = <ThrowOnError extends boolean = false>(options: Options<GetOppgaveData, ThrowOnError>) => (options.client ?? client).get<GetOppgaveResponses, GetOppgaveErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/oppgave/{oppgaveId}',
    ...options
});

export const hentOppgaverForSaksbehandler = <ThrowOnError extends boolean = false>(options?: Options<HentOppgaverForSaksbehandlerData, ThrowOnError>) => (options?.client ?? client).get<HentOppgaverForSaksbehandlerResponses, HentOppgaverForSaksbehandlerErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/oppgave/saksbehandler',
    ...options
});

export const hentOppgaverForPerson = <ThrowOnError extends boolean = false>(options: Options<HentOppgaverForPersonData, ThrowOnError>) => (options.client ?? client).get<HentOppgaverForPersonResponses, HentOppgaverForPersonErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/oppgave/person/{maskertPersonIdent}',
    ...options
});

export const getKodeverkHjemler = <ThrowOnError extends boolean = false>(options?: Options<GetKodeverkHjemlerData, ThrowOnError>) => (options?.client ?? client).get<GetKodeverkHjemlerResponses, GetKodeverkHjemlerErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/klage/kodeverk/hjemler',
    ...options
});

export const getKodeverkHjemlerForYtelse = <ThrowOnError extends boolean = false>(options: Options<GetKodeverkHjemlerForYtelseData, ThrowOnError>) => (options.client ?? client).get<GetKodeverkHjemlerForYtelseResponses, GetKodeverkHjemlerForYtelseErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/klage/kodeverk/hjemler/{ytelseId}',
    ...options
});

export const lastnedDokumentFraJournalpost = <ThrowOnError extends boolean = false>(options: Options<LastnedDokumentFraJournalpostData, ThrowOnError>) => (options.client ?? client).get<LastnedDokumentFraJournalpostResponses, LastnedDokumentFraJournalpostErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/journalpost/{journalpostId}/{dokumentId}',
    ...options
});

export const hentJournalpostMetaData = <ThrowOnError extends boolean = false>(options: Options<HentJournalpostMetaDataData, ThrowOnError>) => (options.client ?? client).get<HentJournalpostMetaDataResponses, HentJournalpostMetaDataErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/journalpost/{journalpostId}/metadata',
    ...options
});

export const finnJournalposterForSak = <ThrowOnError extends boolean = false>(options: Options<FinnJournalposterForSakData, ThrowOnError>) => (options.client ?? client).get<FinnJournalposterForSakResponses, FinnJournalposterForSakErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/journalpost/sak/{saksnummer}',
    ...options
});

export const finnJournalposterForBruker = <ThrowOnError extends boolean = false>(options: Options<FinnJournalposterForBrukerData, ThrowOnError>) => (options.client ?? client).get<FinnJournalposterForBrukerResponses, FinnJournalposterForBrukerErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/journalpost/person/{maskertPersonIdent}',
    ...options
});
